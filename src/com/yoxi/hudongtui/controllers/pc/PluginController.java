...
//		inv.getRequest().setAttribute("brandCount", brandCount);
	 ...
	 */
	@Get("/pluginList")
	public String getPluginList(Invocation inv, @Param("typeFlag") String typeFlag, @Param("orderFlag") String orderFlag, @Param("publishTime_Flag") String publishTime_Flag,
			@Param("buyNum_Flag") String buyNum_Flag, @Param("price_Flag") String price_Flag) throws Exception {
	...
	@Get("ajaxList")
	public String getPluginList(Invocation inv, @Param("typeFlag") String typeFlag, @Param("orderFlag") String orderFlag, @Param("publishTime_Flag") String publishTime_Flag,
			@Param("buyNum_Flag") String buyNum_Flag, @Param("price_Flag") String price_Flag, @Param("start") final int start, @Param("count") int count) throws Exception {
		...
		String cond = " AND a.id NOT IN (SELECT d.pluginId FROM t_plugin_rec d) " + orderStrCon;
		List<PluginDetailVO> pluginList = pluginService.getAgentPluginList(agentInfo.getId(), cond, start, Globals.PC_PLUGINLIST_NUM);
	 ...
	 */
	@Get("/pluginPics")
	public String getPluginePics(Invocation inv, @Param("pluginId") Integer pluginId) throws Exception {
	 ...
	 */
	@Get("/detail/{pluginId}")
	public String getPluginDetail(Invocation inv, @Param("pluginId") Integer pluginId) throws Exception {
		...
		// 1.插件详细信息
		PluginDetailVO plugin = pluginService.getAgentPlugin(pluginId, agentInfoVO.getId());
		...
		// 3.该插件正在进行的活动
		String condition = " AND a.agentId = " + agentInfoVO.getId() + " AND a.pluginId =" + pluginId + " AND NOW() <= b.overdueTime " + " AND a.browseNum >= " + Globals.ACT_BROWSERNUM_LIMIT
				+ " ORDER BY a.browseNum DESC";
		List<PluginActCenterVo> pluginActVos = pluginActService.findActVos(condition);
		...
		// 6.开发者的其他插件
		String condition1 = " and a.userId = " + plugin.getUserId() + " and a.id !=" + pluginId + " ORDER BY a.publishTime DESC LIMIT 0,3";
		List<PluginDetailVO> otherPluginVos = pluginService.findPluginesByCondition(condition1);
		inv.getRequest().setAttribute("otherPluginVos", otherPluginVos);
		inv.getRequest().setAttribute("otherPluginVoCount", otherPluginVos.size());
	 ...
	 */
	@Get("/tjPlugins")
	public String getTjPlugins(Invocation inv, @Param("pluginId") Integer pluginId) throws Exception {
		// 推荐插件
	 ...
	 */
	@Post("tryout")
	public String tryout(Invocation inv, @Param("pluginId") Integer plugingId) throws Exception {
		// 判断是否登录
		...
		Map<String, String> dataMap = pluginBusService.tryOut(new Plugin(plugingId), SessionUtil.getUser(inv.getRequest()).getUserId());
		return "@json:" + JsonUtils.toJson(dataMap);
	}
	 ...
	 */
	@Post("tobuy")
	public String tobuy(Invocation inv, @Param("pluginId") Integer plugingId) throws Exception {
		if (SessionUtil.getUser(inv.getRequest()) == null) {
			Map<String, String> dataMap = new HashMap<String, String>();
			dataMap.put("status", "2");
			return "@json:" + JsonUtils.toJson(dataMap);
		}
		Map<String, String> dataMap = pluginBusService.prepareBuy(new Plugin(plugingId), SessionUtil.getUser(inv.getRequest()).getUserId());
		return "@json:" + JsonUtils.toJson(dataMap);
	}
	 ...
	 */
	@Post("freeUse")
	public String freeUse(Invocation inv, @Param("pluginId") Integer plugingId) throws Exception {
		if (SessionUtil.getUser(inv.getRequest()) == null) {
			Map<String, String> dataMap = new HashMap<String, String>();
			dataMap.put("status", "2");
			return "@json:" + JsonUtils.toJson(dataMap);
		}
		Map<String, String> dataMap = pluginBusService.freeUser(new Plugin(plugingId), SessionUtil.getUser(inv.getRequest()).getUserId());
		return "@json:" + JsonUtils.toJson(dataMap);
	}
	 ...
	 */
	@Post("calculatePrice")
	public String calculatePrice(@Param("mon") Integer mon, @Param("pid") Integer pid, @Param("agid") Integer agid) throws Exception {
		// 找出当前定价
