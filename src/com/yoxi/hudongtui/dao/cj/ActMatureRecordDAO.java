 ...
 */
@DAO
public interface ActMatureRecordDAO {

	@SQL("insert into t_act_mature_record (activityId,#if(:1.tel){tel,}#if(:1.username){username,}" +
	...
	public Identity add(ActMatureRecord actMatureRecord);
	
	@SQL("update t_act_mature_record set #if(:1.tel){tel=:1.tel,}#if(:1.username){username=:1.username,}" +
	...
	public int update(ActMatureRecord actMatureRecord);
	
	@SQL("update t_act_mature_record set ##(:key) = :value where id = :1.id")
	public int updateByCriteria(ActMatureRecord actMatureRecord, @SQLParam("key") String key, @SQLParam("value") String value);
	
	@SQL("SELECT * FROM t_act_mature_record where activityId = :1 order by id desc LIMIT :2,:3")
	public List<MatureRecordExVo> getRecord(Integer activityId, int startRow, int pageSize);
	
	@SQL("SELECT count(*) FROM t_act_mature_record where activityId = :1")
	public int getRecordCount(Integer activityId);
	
	@SQL("select * from t_act_mature_record where activityId = :1 order by id asc LIMIT :2,:3")
	public List<ActMatureRecord> findAll(Integer activityId, int startRow, int pageSize);
	
	@SQL("select count(*) from t_act_mature_record where activityId = :1")
	public int countAllPrizeRecord(Integer activityId);
	
	@SQL("update t_act_mature_record set opState = :2 #if(:3){,exchangeTime = :3}#else{,exchangeTime = NULL} where id = :1")
	public int updateOpStatus(Integer id, String status, Date date);
	
	@SQL("select opState from t_act_mature_record where id = :1")
