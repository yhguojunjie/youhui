...
$(document).ready(function(){
	$('.actrule_msgbox textarea'). focus(function(){
		...
		if($('.suc_count').val()){
			actrule_msgbox += '• 每盘连胜' + $('.suc_count').val() + '局获奖。\n';
		}// else{
		// 	actrule_msgbox += '• 每人每天最多游戏1次。\n';
		// }
		actrule_msgbox += '• 每个好友每天都可跟庄家比一次牌。\n';
...
//新增虚假记录
$(document).ready(function(){
	$(document).on("click",".poker .mark h4 a.bg_green",function(){
		$.ajax({
		  type: "POST",
		  url : path+"/pluginadmin/poker/getPrizeJson",
...
function popupFalsemark(dataarr){
	...
	$('.add_falsemark .close,.add_falsemark .btn_cancel,.add_falsemark .btn_done').click(function(){
		...
		$.ajax({
			  type: "POST",
			  url : path+"/pluginadmin/poker/addRecord",
			  data:jsondata,
			  dataType: "json",
			  success: function(data){
			  	if(data != 0){
...
function addscreen_json(obj){
    var screen1 = {} ;
    var prizeType,prizeName,showNum,realNum,picUrl,exchangeState;
