...
//兑换奖品
$(document).ready(function(){
	$(document).on("click",".runman .mark .btn_green,.runman .mark .btn_gray,.runman .mark .btn_modify",function(){
	...
	$('.prompt_box .close,.prompt_box .btn_cancel,.prompt_box .btn_done').click(function(){
		if($(this).attr("class") == "btn_done"){
			...
			if(type == 1){
				$.ajax({
					  type: "POST",
					  url : path+"/pluginadmin/runman/changeOpStatus",
				...
				$.ajax({
					  type: "POST",
					  url : path+"/pluginadmin/runman/updateBestScore",
