		...
		if($(this).prev().find('h4 strong').text().substring(0,1)*1 < 6){
			$(this).before('<div class="screen" data-id="'+($(this).prev().find('h4 strong').text().substring(0,1)*1 + 1) +'">\
							...
							<div class="row">\
								...
								<div class="fr">\
									<p>\
										<label><input class="act_ticket" type="radio" name="gift2" value="0">是</label>&nbsp;&nbsp;\
										<label><input type="radio" name="gift2" value="1" checked="checked">否</label>\
...
//兑换奖品
$(document).ready(function(){
	$(document).on("click",".invoice .mark .btn_green,.invoice .mark .btn_gray,.invoice .mark .btn_modify",function(){
	...
	$('.prompt_box .close,.prompt_box .btn_cancel,.prompt_box .btn_done').click(function(){
		if($(this).attr("class") == "btn_done"){
			...
			if(type == 1){
				$.ajax({
					  type: "POST",
					  url : path+"/pluginadmin/invoice/changeOpStatus",
				...
				$.ajax({
					  type: "POST",
					  url : path+"/pluginadmin/invoice/updateBestScore",
