	...
	<div class="w1190 plugin_detail">
		...
		<div class="fl w840">
			<div class="main">
							...
							<div class="btns">
								...
								<c:choose>
									...
									<c:when test="${plugin.salePrice != null &&  plugin.salePrice == 0.0 }">
										...
										<a href="javascript:void(0);" onclick="tobuy('${plugin.id}');" class="btn_direct">价格面议</a>
									</c:when>
								</c:choose>
							</div>
						</dd>
				...
				<c:if test="${(plugin.guide!=null)&&(plugin.guide !='')}"> 
				...
				<c:if test="${plugin.videoUrl != null && plugin.videoUrl != ''}">
...
<!-- 					<span class="suc_gold"><i class="ico_suc"></i>成交：<strong>250</strong>笔</span> -->
	...
	<div class="pay_box" id="paysuc">
		<form action="${path }/pay/order/down" method="post" name="payForm"
			...
			<div class="clear space_20"></div>
			<c:if test="${plugin.salePrice != null && plugin.salePrice == 0.0}">
				<div class="row">请先提交订单，稍后与我们联系，协商定价</div>
				<div class="clear space_20"></div>
			</c:if>
			<div class="row">
				有效期
			...
			<c:choose>
				<c:when test="${plugin.salePrice != null && plugin.salePrice != 0.0}">
					<h4>消费：<span>￥<del style="display:none;" id="useMoneyOld">${priceNoDot}</del>&nbsp;<em id="useMoney">${priceNoDot}</em></span></h4>
					<div class="row">
