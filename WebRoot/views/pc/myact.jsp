...
<body>
	...
	<div class="pay_box" id="paysuc">
		<form action="${path }/pay/pay/pf" method="post" name="payForm"
			...
			<h4>消费：<span id="useMoney"></span></h4>
			<div class="row">
			     	 ...
			     	 if(data.pageResult != undefined && data.pageResult != null && data.pageResult.length != 0){
			     		 ...
			     		 for (var i = 0; i < data.pageResult.length; i++) {
						    ...
						    //	html += '<textarea style="display:none" id="mytext">'+cjpcShowServer+data.pageResult[i].actAccessUrl+'/'+data.pageResult[i].actId+'</textarea>';
						    	...
						    	html += '<p><button id="copy'+data.pageResult[i].actId+'" class="btn_copy" value="'+cjpcShowServer+data.pageResult[i].actAccessUrl+'/'+data.pageResult[i].actId+'" onclick="init(\'copy'+data.pageResult[i].actId+'\')">双击复制地址</button></p>';
						     	html += '</div>';
					     	}
