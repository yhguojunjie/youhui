			...
			<div class="tab_title">
				<c:choose>
					<c:when test="${userPluginId != null && userPluginId != 0}">
						<a href="${path}/pluginadmin/ripbrand/add/${userPluginId}">活动设置</a>
					</c:when>
					<c:otherwise>
						<a href="${path}/pluginadmin/ripbrand/edit/${activityId}">活动设置</a>
					</c:otherwise>
				</c:choose>
				...
				<h4 class="tr">
					<script type="text/javascript">
					 function exportRecord(){
						 ...
						 window.location.href="${path}/pluginadmin/ripbrand/exportRecord/${activityId}?start="+start+"&end="+end;
					 }
					</script>
					<i>请输入导出条数：从</i>
