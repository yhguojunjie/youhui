...
<link type="text/css" rel="stylesheet" href="${adminCssPath}/vote.css?v=1"/>
		...
		<div class="fl w840 main">
			<div class="tab_title">
				<c:choose>
					<c:when test="${userPluginId != null && userPluginId != 0}">
						<a href="${path}/pluginadmin/vote/add/${userPluginId}">活动设置</a>
					</c:when>
					<c:otherwise>
						<a href="${path}/pluginadmin/vote/edit/${activityId}">活动设置</a>
					</c:otherwise>
				</c:choose>
			...
			<div class="cont mark">
				<h4 class="tr">
						<script type="text/javascript">
						 function exportRecord(){
							 ...
							 window.location.href="${path}/pluginadmin/vote/exportRecord/${activityId}?start="+start+"&end="+end;
						 }
						</script>
					<i>请输入导出条数：从</i>
				...
				<table>
					...
					<c:forEach var="list" items="${rankVos.pageResult}" varStatus="s">
						<tr>
							<td>${s.count + count*(currentPage-1)}</td>
							<td>${list.voterId}</td>
							<td><span>${list.voteNum}</span></td>
							...
							<c:if test="${list.otherInfo4 != null && !empty(list.otherInfo4)}">${list.otherInfo4}<br/></c:if>
							</td>
							<td>
								<a class="btn_modify">修改票数</a>
			...
			<!-- 分页 开始 -->
			<div class="tr pages">
				<p:page page="rankVos" formname="recordListForm" />
			</div>
			<!-- 分页 结束 -->
		</div>
		<!-- 左侧内容 结束 -->
