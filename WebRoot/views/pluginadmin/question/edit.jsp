	...
	<div class="w1190 question">
		...
		<div class="fl w840 main">
			<div class="tab_title">
				...
				<a href="${path}/pluginadmin/question/bank/${conf.activityId}">题库管理</a>
				<a href="${path}/pluginadmin/question/record/${conf.activityId}">中奖名单</a>
				<a href="${path}/pluginadmin/question/rank/${conf.activityId}">排行榜</a>
			</div>
			<div class="cont">
				<form action="${path}/pluginadmin/question/edit_do" method="post" onsubmit="return onClick();">
							...
							<div class="fr">
								<div class="preview">
									<c:choose>
										...
										<c:otherwise>
											<img class="w80 h80" src="${adminImgPath}/question/logo.png"/>
										</c:otherwise>
									</c:choose>
								</div>
					...
					<div class="ques_config">
						<div class="row">
							...
							<div class="fr">
								<p>
									<label><input type="radio" name="bQestType" value="0" <c:if test="${conf.bQestType == '0'}">checked</c:if>/>从题库随机选取</label>&nbsp;&nbsp;&nbsp;&nbsp;
									<label><input type="radio" name="bQestType" value="1" <c:if test="${conf.bQestType == '1'}">checked</c:if>/>从题库按顺序选取</label>
								</p>
							</div>
							<div class="clear"></div>
						</div>
						<div class="row">
								...
								<div class="fr">
									<p>
										<input name="questionNum" class="verify_num w80" type="text" value="${conf.questionNum}" />
										<b>不能超过题库题目总数</b>
									</p>
								</div>
								<div class="clear"></div>
							...
							<div class="row">
								...
								<div class="fr">
									<p>
										<input name="questionTime" class="verify_num w80" type="text" value="${conf.questionTime}" />
										<b>秒</b>
									</p>
								</div>
								<div class="clear"></div>
					...
					<!-- 奖品 开始 -->
						...
						<div class="screen" data-id="${s.count}">
									...
									<div class="preview">
										<c:choose>
											...
											<c:when test="${s.count < 4}">
												<img class="w80 h80" src="${adminImgPath}/giftbox_img${s.count}.png" />
											</c:when>
											<c:otherwise>
												<img class="w80 h80" src="${adminImgPath}/question/gift.png" />
											</c:otherwise>
										</c:choose>
									</div>
									<div class="upload_file">
							...
							<div class="row">
								...
								<div class="fr">
									<p>
										<label><input type="radio" name="${s.count}" value="0" <c:if test="${list.exchangeState == '0'}">checked</c:if>/>是</label>&nbsp;&nbsp;
										<label><input type="radio" name="${s.count}" value="1" <c:if test="${list.exchangeState == null || list.exchangeState == '1'}">checked</c:if>
											/>否</label>
						...
						<div class="row"  id="attention_limit" style="margin-bottom:0px;">
							...
							<div class="fr w560">
								<p>
									...
									<label><input type="radio"  name="bUrlType" value="1" <c:if test="${conf.bUrlType == '1'}">checked</c:if>/>无</label>
								</p>
							</div>
							<div class="clear"></div>
						</div>
							...
							<div class="fr w560">
								<div class="preview">
									<c:choose>
										...
										<c:otherwise>  
											<img class="w160 h240" src="${adminImgPath}/question/cover.jpg" />
										</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
							...
							<div class="fr w560">
								<div class="preview">
									<c:choose>
										...
										<c:otherwise>  
											<img class="w160 h240" src="${adminImgPath}/question/mainbg.jpg" />
										</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
