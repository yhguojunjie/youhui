			...
			<div class="tab_title">
				<c:choose>
					<c:when test="${userPluginId != null && userPluginId != 0}">
						<a href="${path}/pluginadmin/mature/add/${userPluginId}">活动设置</a>
					</c:when>
					<c:otherwise>
						<a href="${path}/pluginadmin/mature/edit/${activityId}">活动设置</a>
					</c:otherwise>
				</c:choose>
				...
				<h4 class="tr">
					<script type="text/javascript">
					 function exportRecord(){
						 ...
						 window.location.href="${path}/pluginadmin/mature/exportRecord/${activityId}?start="+start+"&end="+end;
					 }
					</script>
					<i>请输入导出条数：从</i>
				...
				<table>
					...
					<c:forEach var="list" items="${recordVos.pageResult}" varStatus="s">
						<tr>
							...
							<td><span>${list.count}个红包</span></td>
							...
							<td>${list.totalExNum}个红包</td>
							<td>
								<p><fmt:formatDate value="${list.submitTime}" pattern="yyyy-MM-dd HH:mm:ss" /></p>
							</td>
							<td>
