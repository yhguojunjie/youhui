	...
	<div class="w1190 poker">
		...
		<div class="fl w840 main">
			<div class="tab_title">
				...
				<a href="${path}/pluginadmin/poker/join/${conf.activityId}">参与用户</a>
				<a href="${path}/pluginadmin/poker/record/${conf.activityId}">中奖名单</a>
			</div>
			<div class="cont">
				<form action="${path}/pluginadmin/poker/edit_do" method="post" onsubmit="return onClick();">
					...
					<div class="basic_config">
						...
						<div class="row">
							...
							<div class="fr">
								<div class="preview">
									<c:choose>
									<c:when test="${conf.shareImgUrl != null && !empty(conf.shareImgUrl)}">
										<img class="w80 h80" src="${resUrl}/${conf.shareImgUrl}" />
									</c:when>
									<c:otherwise>
										<img class="w80 h80" src="${adminImgPath}/poker/poker_ico.png" />
									</c:otherwise>
									</c:choose>
								</div>
						...
						<div class="row">
							...
							<div class="fr">
								...
								<div class="ad_upcont space_10">
									<div class="preview">
										<c:choose>
											<c:when test="${conf.advertImgUrl != null && !empty(conf.advertImgUrl)}">
												<img class="w320 h50" src="${resUrl}/${conf.advertImgUrl}" />
											</c:when>
											<c:otherwise>
												<img class="w320 h50" src="${adminImgPath}/ad.png" />
											</c:otherwise>
										</c:choose>
									</div>
									<div class="upload_file">
					...
					<div class="cont_config">
						<div class="row">
							...
							<div class="fr w560">
								<p>
									<label class="active"><input type="radio" name="bShowNum" value="0" <c:if test="${conf.bShowNum == '0'}">checked</c:if>/>是</label>&nbsp;&nbsp;
									<label><input type="radio" name="bShowNum" value="1" <c:if test="${conf.bShowNum == '1'}">checked</c:if>/>否</label>
						...
						<div class="row">
							...
							<div class="fr w560">
								<p>
									<label class="active"><input type="radio" name="bShowList" value="0" <c:if test="${conf.bShowList == '0'}">checked</c:if>/>是</label>&nbsp;&nbsp;
									<label><input type="radio" name="bShowList" value="1" <c:if test="${conf.bShowList == '1'}">checked</c:if>/>否</label>
									<b>奖品显示在活动界面的展示数量（仅供用户观看）</b>
								</p>
							</div>
							<div class="clear"></div>
						...
						<div class="screen">
									...
									<div class="preview">
										<c:choose>
											<c:when test="${list.picUrl != null && !empty(list.picUrl)}">
												<img class="w80 h80" src="${resUrl}/${list.picUrl}" />
											</c:when>
											<c:otherwise>
												<img class="w80 h80" src="${adminImgPath}/toothbrush.gif" />
											</c:otherwise>
										</c:choose>
									</div>
									<div class="upload_file">
							...
							<div class="row">
								...
								<div class="fr">
									<p>
										<label><input type="radio" name="change_gift" value="0" <c:if test="${list.exchangeState == '0'}">checked</c:if>/>是</label>&nbsp;&nbsp;
										<label><input type="radio" name="change_gift" value="1" <c:if test="${list.exchangeState == null || list.exchangeState == '1'}">checked</c:if>/>否</label>
					...
					<div class="act_config">
						<div class="row"  id="attention_limit" style="margin-bottom:0;">
							...
							<div class="fr w560">
								<p>
									...
									<label><input type="radio" name="bUrlType" value="1" <c:if test="${conf.bUrlType == '1'}">checked</c:if>/>兑换奖品</label>
								...
								<div class="space_10"></div>
									...
									<div class="fr w560">
										<p>
											...
											<b>用户打开好友发送或分享至朋友圈的链接，欲参与活动会先跳转该链接。建议用公众号新增图文素材，内容为活动规则，重点引导用户关注，再在公众号内设置关键词回复，如用户发送“坐庄”，公众号将回复活动链接地址（在PC端我的活动列表获取，该状态下仅在被分享出去的页面生效，原活动链接地址打开的页面不生效)。</b>
										</p>
									</div>
								<div class="clear"></div>
							</div>
							...
							<div class="row" id="followDiv">
								<div class="space_10"></div>
									...
									<div class="fr w560">
										<p>
											<input name="followUrl" class="verify_contact1 w420" type="text" value="${conf.followUrl}" /><br />
											<b>参与用户要查看排行榜页(自己的名次、领奖)，会先跳转该链接。建议用公众号新增图文素材，内容为活动规则，重点引导用户关注，再在公众号内设置关键词回复，如用户发送“坐庄”，公众号将回复排行榜页链接地址（在PC端活动配置排行榜栏目获取)。</b>
										</p>
									</div>
								<div class="clear"></div>
							</div>
						...
						<div class="row">
							<strong class="fl w220">每盘几局连胜获奖：</strong>
							<div class="fr w560">
								<p>
									<input name="playTimes" class="verify_num w80 suc_count" type="text" value="${conf.playTimes}" />
									<b>大于0</b>
								</p>
							</div>
							<div class="clear"></div>
					...
					<div class="custom_config">
						<div class="row">
							...
							<div class="fr w560">
								<div class="preview">
									<c:choose>
									<c:when test="${conf.coverUrl != null && !empty(conf.coverUrl)}">
										<img class="w160 h240" src="${resUrl}/${conf.coverUrl}" />
									</c:when>
									<c:otherwise>
										<img class="w160 h240" src="${adminImgPath}/poker/cover.jpg" />
									</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
						...
						<div class="row">
							...
							<div class="fr w560">
								<div class="preview">
									<c:choose>
									<c:when test="${conf.bgUrl != null && !empty(conf.bgUrl)}">
										<img class="w160 h240" src="${resUrl}/${conf.bgUrl}" />
									</c:when>
									<c:otherwise>
										<img class="w160 h240" src="${adminImgPath}/poker/bg.jpg" />
									</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
...
<script type="text/javascript">
	...
	function verify_submit(){
		...
		if(verify > 0){
			$('.poker .main .cont .imgs_config,.poker .main .cont .cont_config,.poker .main .cont .act_config,.poker .main .cont .custom_config').attr("style","display:block;");	
			$('.poker .main .cont h3 span').text("收起");
			alert("请核查全部字段是否正确填写");
			return false;
		}else{
			return true;
