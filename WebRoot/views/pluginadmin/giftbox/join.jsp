...
<script type="text/javascript" src="${adminJsPath}/giftbox.js?v=2"></script>
			...
			<div class="tab_title">
				<c:choose>
					<c:when test="${userPluginId != null && userPluginId != 0}">
						<a href="${path}/pluginadmin/giftbox/add/${userPluginId}">活动设置</a>
					</c:when>
					<c:otherwise>
						<a href="${path}/pluginadmin/giftbox/edit/${activityId}">活动设置</a>
					</c:otherwise>
				</c:choose>
				<a class="active" href="#">参与用户</a>
				<a href="${path}/pluginadmin/giftbox/record/${activityId}">中奖名单</a>
			</div>
			<div class="cont mark">
				<h4 class="tr">
						<script type="text/javascript">
						 function exportJoin(){
							 ...
							 window.location.href="${path}/pluginadmin/giftbox/exportJoin/${activityId}?start="+start+"&end="+end;
						 }
						</script>
					<i>请输入导出条数：从</i>
				...
				<table>
					...
					<c:forEach var="list" items="${recordVos.pageResult}" varStatus="s">
						<tr>
							...
							<td>
								<c:choose>
									<c:when test="${list.prizeType == null || list.prizeType == 0}">
										<p>否</p>
									</c:when>
									<c:otherwise>
										<p>是</p>
