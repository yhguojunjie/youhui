...
<body>
	...
	<div class="w1190 takepic">
		...
		<div class="fl w840 main">
			...
			<div class="cont">
				<form action="${path}/pluginadmin/takepic/edit_do" method="post" onsubmit="return onClick();">
					...
					<div class="basic_config">
						<div class="row">
							...
							<div class="fr">
								<p>
									<input class="verify_name w300" type="text" name="name" maxlength="30" value="${conf.name}" />
								</p>
							</div>
							<div class="clear"></div>
						</div>
							...
							<div class="fr">
								...
								<c:choose>
									<c:when test="${conf.shareImgUrl != null && conf.shareImgUrl.length() !=0}">
										<img class="w80 h80 imghead" src="${resUrl}/${conf.shareImgUrl}" />
									</c:when>
									<c:otherwise>
										<img class="w80 h80 imghead" src="${adminImgPath}/takepic/logo.png" />
									</c:otherwise>
									</c:choose>
								</div>
							...
							<div class="fr">
								...
								<div class="ad_upcont space_10">
									...
									<c:choose>
										...
										<c:otherwise>  
											<img class="w320 h70 imghead" src="${adminImgPath}/ad.gif" />
										</c:otherwise>
										</c:choose>
									</div>
									<div class="upload_file">
									...
									<p>
										<input class=" w420" type="text" value="${conf.advertUrl}" name="advertUrl"/>
									</p>
								</div>
							</div>
							<div class="clear"></div>
					...
					<div class="act_config">
						...
						<div class="row">
							...
							<div class="fr">
								<p>
									<input class="verify_contact2 w420" type="text" value="${conf.actUrl}" name="actUrl">
									<b></b>
								</p>
							</div>
							<div class="clear"></div>
					...
					<div class="custom_config">
							...
							<div class="fr w560">
								...
								<c:choose>
										<c:when test="${conf.coverUrl != null && conf.coverUrl.length() !=0}">
											<img style="background:#eee;" class="w320 h200" src="${resUrl}/${conf.coverUrl}" />
										</c:when>
										<c:otherwise>  
											<img style="background:#eee;" class="w320 h200" src="${adminImgPath}/takepic/img.png" />
										</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
						...
						<div class="row">
							...
							<div class="fr w560">
								<div class="preview">
								<c:choose>
										<c:when test="${conf.normalUrl != null && conf.normalUrl.length() !=0}">
											<img style="background:#eee;" class="w120 h120" src="${resUrl}/${conf.normalUrl}" />
										</c:when>
										<c:otherwise>  
											<img style="background:#eee;" class="w120 h120" src="${adminImgPath}/takepic/phone-0.png" />
										</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
									<input type="text" class="fileuploadclass" name="normalUrl" value="${conf.normalUrl}">
									<span><em class="ico_image"><!-- 图标 --></em>选择背景</span>
								</div>
						...
						<div class="row">
							...
							<div class="fr w560">
								<div class="preview">
								<c:choose>
										<c:when test="${conf.doubleUrl != null && conf.doubleUrl.length() !=0}">
											<img style="background:#eee;" class="w120 h120" src="${resUrl}/${conf.doubleUrl}" />
										</c:when>
										<c:otherwise>  
											<img style="background:#eee;" class="w120 h120" src="${adminImgPath}/takepic/phone-1.png" />
										</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
									<input type="text" class="fileuploadclass" name="doubleUrl" value="${conf.doubleUrl}">
									<span><em class="ico_image"><!-- 图标 --></em>选择背景</span>
								</div>
								<b>建议尺寸：200*200（或等比）,PNG格式，大小不超过30K。</b>
							</div>
						...
						<div class="row">
							...
							<div class="fr w560">
								...
								<c:choose>
										...
										<c:otherwise>  
											<img class="w160 h240" src="${adminImgPath}/takepic/bg.jpg" />
										</c:otherwise>
									</c:choose>
								</div>
								<div class="upload_file">
